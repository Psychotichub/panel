...
async function initPanelModule() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                panels
                    ...
                    .forEach((panel) => {
                        panelsList.push(panel.panelName);
                        addPanelToTable(panel);
                    });
                updatePanelList();
        ...
        submitButton.addEventListener('click', async () => {
            ...
            if (isEditing) {
                ...
                try {
                    ...
                    if (response.ok) {
                        panelsList.push(panelName);
                        updatePanelList();
                        addPanelToTable(result);
    ...
    function addPanelToTable(panel) {
        ...
        if (canManagePanels) {
            ...
            deleteButton.addEventListener('click', async () => {
                if (confirm(`Are you sure you want to delete ${panel.panelName}?`)) {
                    try {
                        ...
                        if (response.ok) {
                            row.remove();
                            const index = panelsList.indexOf(panel.panelName);
                            if (index >= 0) {
    ...
    function filterPanels(searchTerm) {
        ...
        rows.forEach((row) => {
            const panelName = row.querySelector('td').textContent.toLowerCase();
            if (panelName.includes(searchTerm.toLowerCase())) {
                row.style.display = '';
            } else {
